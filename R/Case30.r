...
#' parameters following Case 30 and employing a sampling resolution of
#' \code{0.1 mm}, \code{0.2 mm}, \code{0.45 mm}, \code{0.75 mm}, \code{1.55 mm}
#' and \code{3.25 mm}.
#' 
#' Case 30 describes an ideal temperature sinusoid without distortion by changes
#' in growth rate, but with seasonal variability in \eqn{\delta^{18}O_{w}}{δ18Ow}
#' with a minimum in spring season and a small (5 days SD) error on age model
#' (roughly +/– one week)
#' 
#' Generated using the code in "Generate_Case30.r" in \code{data–raw}
#'
