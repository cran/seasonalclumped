...
#' parameters following Case 33 and employing a sampling resolution of
#' \code{0.1 mm}, \code{0.2 mm}, \code{0.45 mm}, \code{0.75 mm}, \code{1.55 mm}
#' and \code{3.25 mm}.
#' 
#' Case 33 describes an ideal temperature sinusoid without distortion by changes
#' in growth rate, but with seasonal variability in \eqn{\delta^{18}O_{w}}{δ18Ow}
#' with a minimum in spring season and a enormous (90 days SD) error on age
#' model (roughly +/– half year)
#' 
#' Generated using the code in "Generate_Case33.r" in \code{data–raw}
#'
